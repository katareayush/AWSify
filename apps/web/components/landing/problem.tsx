...
export function Problem() {
  return (
    <Section id="problem" eyebrow="The problem" title="Shipping to AWS is still painful.">
      <div className="mt-10 grid gap-px overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.03] sm:mt-14 sm:grid-cols-2 sm:rounded-2xl">
        {pains.map((p) => (
...
function PainCard({ title, body }: { title: string; body: string }) {
  return (
    <div className="group relative overflow-hidden bg-[#070708] p-5 transition-colors hover:bg-[#0b0b0e] sm:p-7">
      ...
      <h3 className="mt-4 text-[19px] font-medium tracking-tight text-white sm:text-[22px]">{title}</h3>
